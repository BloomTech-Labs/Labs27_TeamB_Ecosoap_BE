...
router.post('/', (req,res) => {
    const { organizationName, contactName, barsRequested, 
            contactEmailAddress, country, beneficiaries }
            = req.body
    ...
    .then(data => {
        ...
        // depending on what we get form the result, if we get 

        if (data.data.checkIfPrice.hasPrice === true){
            // console.log('TRUEEEEEEEEEEEE')
            
            res.status(200).json({data:data, message:`Your Order for quantity of ${barsRequested} bars of soap will be a total of $ ${data.data.checkIfPrice.price}`})
